...
class NuScenesDataset(Dataset):
    ...
    def __init__(self, dataroot, nuscenes, mode='train', seconds_of_history=1, dist='bos', path_to_epsilon_set = "../../data/nuscenes/covernet_traj_set/nuscenes-prediction-challenge-trajectory-sets/epsilon_8.pkl"):
        ...
        self.agent_rasterizer = AgentBoxesWithFadedHistorySeparate(self.helper, seconds_of_history=self.seconds_of_history)
        self.input_representation = InputRepresentationDecode(self.static_layer_rasterizer, self.agent_rasterizer, Rasterizer())
    ...
    def __getitem__(self, idx):
        ...
        center_agent, hd_map, social_context, combined = self.input_representation.make_input_representation(instance_token, sample_token)
        center_agent = torch.Tensor(center_agent).permute(2, 0, 1).unsqueeze(0)
        hd_map = torch.Tensor(hd_map).permute(2, 0, 1).unsqueeze(0)
        social_context = torch.Tensor(social_context).permute(2, 0, 1).unsqueeze(0)
        combined = torch.Tensor(combined).permute(2, 0, 1).unsqueeze(0)
        ...
        past = self.helper.get_past_for_agent(instance_token, sample_token, seconds=self.seconds_of_history, in_agent_frame=True)
        agent_state_vector = torch.cat((agent_state_vector.flatten(), torch.Tensor(past.flatten())))
        agent_state_vector = torch.unsqueeze(agent_state_vector, dim=0)
        ...
        return center_agent, hd_map, social_context, agent_state_vector, combined, label
