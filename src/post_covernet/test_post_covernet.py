...
from post_covernet import PostCoverNet, UCELoss

from dataset_ood_norm_input_utils import *
from model_ood_norm_input_utils import *
...
def main(opts):
    ...
    model_name = 'post_covernet_agent_norm_10'
    ...
    for img, agent_state_vector, ground_truth in tq.tqdm(dataset.test):
        logits = model_postcovernet_test.forward((img.cuda(), agent_state_vector.cuda()), return_output='alpha')
    ...
    pkl.dump(ground_truth_preds_test, open(model_name + "_ground_truth_preds_test.pkl", "wb"))
    pkl.dump(covernet_preds_test, open(model_name + "_preds_test.pkl", "wb"))
    pkl.dump(logits_test, open(model_name + "_logits_test.pkl", "wb"))
    ...
    # Save the ground truth and predictions
    pkl.dump(ground_truth_preds_test, open(model_name + "_ground_truth_preds_ood_test_last.pkl", "wb"))
    pkl.dump(logits_test, open(model_name + "_logits_ood_test_last.pkl", "wb"))

    # Compute out-of-distribution uncertainty metrics.
    ground_truth_preds_test_in = pkl.load(open(model_name + "_ground_truth_preds_ood_test_last.pkl", "rb"))
    logits_test_in = pkl.load(open(model_name + "_logits_test_last.pkl", "rb"))
    logits_test_ood = pkl.load(open(model_name + "_logits_ood_test_last.pkl", "rb"))
